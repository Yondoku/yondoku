...
if (!args.length) {
  const messageboardEmbed = new Discord.MessageEmbed().setColor('#5d83a2').setAuthor(`${message.author.tag}`, `${message.author.avatarURL()}`).setTitle('Message board says...').setDescription(fs.readFileSync('commands/messageboard.txt'));
...
fs.writeFileSync('commands/messageboard.txt', args.join(" "));
